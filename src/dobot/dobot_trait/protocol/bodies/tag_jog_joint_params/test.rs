...
mod tests {
    use crate::dobot::dobot_trait::protocol::{
        Body, bodies::tag_jog_joint_params::TagJOGJointParams, protocol_error::ProtocolError,
    };

    /// Test case for successful serialization and deserialization of TagJOGJointParams.
    #[test]
    fn test_tag_jog_joint_params_pack_unpack_success() {
        // Create an original struct instance with sample float values
        let original_params = TagJOGJointParams {
        ...
        let deserialized_params = TagJOGJointParams::deserialize(&buffer[..size]).unwrap();
    ...
    #[test]
    fn test_tag_jog_joint_params_unpack_buffer_too_small() {
        // Create a buffer that is intentionally too small
        let buffer = [0u8; 31];
        let result = TagJOGJointParams::deserialize(&buffer);
