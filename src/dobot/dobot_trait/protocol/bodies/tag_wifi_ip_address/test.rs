...
mod tests {
    use crate::dobot::dobot_trait::protocol::{
        Body, bodies::tag_wifi_ip_address::TagWIFIIPAddress, protocol_error::ProtocolError,
    };
    // The Ipv4Addr import is no longer needed since we are using a fixed-size array directly.

    /// Test case for successful serialization and deserialization of TagWIFIIPAddress.
    #[test]
    fn test_tag_wifi_ip_address_pack_unpack_success() {
        // Create an original struct instance with sample values
        // Now directly initialize with a fixed-size array
        let original_cmd = TagWIFIIPAddress {
            dhcp: true,
            addr: [192, 168, 1, 100],
        };
        ...
        // Assert that the size is correct (1 u8 + 1 Ipv4Addr = 5 bytes)
        assert_eq!(size, 5);

        // Deserialize the bytes back into a new struct instance
        let deserialized_cmd = TagWIFIIPAddress::deserialize(&buffer[..size]).unwrap();
    ...
    #[test]
    fn test_tag_wifi_ip_address_unpack_buffer_too_small() {
        // Create a buffer that is intentionally too small
        let buffer = [0u8; 4];
        let result = TagWIFIIPAddress::deserialize(&buffer);
