...
mod tests {
    use crate::dobot::dobot_trait::protocol::{
        Body, ProtocolError, bodies::tag_ptp_common_params::TagPTPCommonParams,
    };

    /// Test case for successful serialization and deserialization of TagPTPCommonParams.
    #[test]
    fn test_tag_ptp_common_params_pack_unpack_success() {
        // Create an original struct instance with sample float values
        let original_params = TagPTPCommonParams {
            velocity_ratio: 0.75,
        ...
        let deserialized_params = TagPTPCommonParams::deserialize(&buffer[..size]).unwrap();
    ...
    #[test]
    fn test_tag_ptp_common_params_unpack_buffer_too_small() {
        // Create a buffer that is intentionally too small
        let buffer = [0u8; 7];
        let result = TagPTPCommonParams::deserialize(&buffer);
